...
function mysql_ins {
	...
	/www/wdlinux/mysql/bin/mysql -uroot -p"wdlinux.cn" -e"use mysql;update user set password=password('wdlinux.cn') where user='root';delete from user where user='';drop user ''@'%';flush privileges;"
...
function apache_ins {
	...
	cd httpd-$APA_VER
	make_clean
	./configure --prefix=$IN_DIR/httpd-$APA_VER --enable-rewrite --enable-deflate --disable-userdir --enable-so --enable-expires --enable-headers --with-included-apr --with-apr=/usr --with-apr-util=/usr --enable-ssl --with-ssl=/usr
