...
if [ $OS_RL == 2 ]; then
    apt-get update
    apt-get remove -y apache2 apache2-utils apache2.2-common \
    apache2.2-bin apache2-mpm-prefork apache2-doc apache2-mpm-worker \
    mysql-common mysql-client mysql-server php5 php5-fpm pure-ftpd \
    pure-ftpd-common pure-ftpd-mysql
    apt-get -y autoremove
    apt-get install -y gcc g++ make autoconf libltdl-dev \
        ...
        libreadline-dev bzip2 libcap-dev ntpdate chkconfig
    if [ $X86 == 1 ]; then
    ...
    yum install -y gcc gcc-c++ make sudo autoconf \
    libtool-ltdl-devel gd-devel freetype-devel \
    libxml2-devel libjpeg-devel libpng-devel \
    openssl-devel curl-devel patch libmcrypt-devel \
    libmhash-devel ncurses-devel bzip2 libcap-devel ntp sysklogd
    if [ $X86 == 1 ]; then
