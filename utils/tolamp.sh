...
if [ $OS_RL == 2 ];then
        update-rc.d nginxd disable 235
else
        chkconfig --level 35 nginxd off
fi
fi
...
function nginx_ins {
	...
	cd $IN_SRC
	tar zxvf nginx-1.0.12.tar.gz > $IN_LOG 2>&1
	cd nginx-1.0.12
	make_clean
	./configure --user=www --group=www --prefix=$IN_DIR/nginx-1.0.12 --with-http_stub_status_module --with-http_ssl_module
	[ $? != 0 ] && err_exit "nginx configure err"
	make
	[ $? != 0 ] && err_exit "nginx make err"
	make install
	[ $? != 0 ] && err_exit "nginx make install err"
	ln -sf $IN_DIR/nginx-1.0.12 $IN_DIR/nginx
...
function php_ins {
	...
	if [ $SERVER == "nginx" ];then
        	ln -sf $IN_DIR/apache_php/sbin/php-fpm $IN_DIR/init.d/php-fpm
	        chmod 755 $IN_DIR/init.d/php-fpm
	        ln -sf $IN_DIR/apache_php/sbin/php-fpm /etc/init.d/php-fpm
		if [ $OS_RL == 2 ];then
...
if [ ! -d /www/wdlinux/apache ];then
        ...
        cp -f /root/conf/defaulta.conf /www/wdlinux/apache/conf/vhost/00000.default.conf
	cp -f /root/conf/wdlinux_a.php /www/web/default/index.php
	sed -i 's/88/80/g' /www/wdlinux/apache/conf/httpd.conf
	sed -i 's/88/80/g' /www/wdlinux/apache/conf/vhost/00000.default.conf
	service httpd restart
fi
php_ins	
eaccelerator_ins
